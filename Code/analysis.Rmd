---
title: "Absences, symptoms and respiratory viruses in a Swiss school: Longitudinal study with serial saliva sampling"
...
# Sens. Analysis: absence matching based on 4day window
if (sens4abs) {
  infections_imp <- readRDS("../data-imputed/linked-infections-absences4.rds") %>%
  mutate(
...
# Sens. Analysis: absence matching based on 10day window
if (sens10abs) {
  infections_imp <- readRDS("../data-imputed/linked-infections-absences10.rds") %>%
  mutate(
    pathogen = factor(pathogen, levels = pathgs)
  )
  source("../utils/dataprep_descr_analysis.R")
...
infections_imp <- readRDS("../data-imputed/linked-infections-absences_ds.rds") %>%
  mutate(
    pathogen = factor(pathogen, levels = pathgs)
  )
source("../utils/dataprep_descr_analysis.R")
...
surv_absences <- infections_imp %>%
  ...
  left_join(soc_data |> dplyr::select(student_id, Gender), by = "student_id")
...
for (i in 1:nrow(symptoms_scaled)) {
  ...
  combined_plot <- arrangeGrob(
    ...
    radar_plot,
    ncol = 1,
    heights = unit.c(grobHeight(title_grob) + ggplot2::unit(0.1, "line"), 
                     ggplot2::unit(1, "npc") - 4*grobHeight(title_grob) - 
                       ggplot2::unit(0.1, "line"))
  )
...
combined_plot_8 <- arrangeGrob(
  ...
  radar_plot_8,
  ncol = 1,
  heights = unit.c(grobHeight(title_grob_8) + ggplot2::unit(0.1, "line"), 
                   ggplot2::unit(1, "npc") - 4*grobHeight(title_grob) - ggplot2::unit(0.1, "line"))
)
...
```{r}
## Sensitivity Analysis Self-reported symptoms
## -----------------------------------------------------------------------------
...
if (!without_coinf) {
  ...
  symp10_scaled <- symp10 %>%
    mutate(across(-pathogen, normalize),
           pathogen = factor(pathogen, levels = pathgs)) %>%
    arrange(pathogen)
  
} else {
  symptoms_scaled_woci <- symptoms_woci %>%
    mutate(across(-pathogen, normalize),
           pathogen = factor(pathogen, levels = pathgs)) %>%
    arrange(pathogen)
}
...
for (i in 1:nrow(symptoms_scaled)) {
  ...
  if (!without_coinf) {
    df_cols <- c("7 days" = "#3474b4", "4 days" = "#d31d1d", "10 days" = "darkgreen") 
    
    df1 <- symptoms_scaled[i, ] %>%
      mutate(dataset = "7 days", pathogen = as.character(pathogen)) %>%
      dplyr::select(dataset, fever:stomach_pains)
    
    df2 <- symp4_scaled[i, ] %>%
      mutate(dataset = "4 days", pathogen = as.character(pathogen)) %>%
      dplyr::select(dataset, fever:stomach_pains)
    
    df3 <- symp10_scaled[i, ] %>%
      mutate(dataset = "10 days", pathogen = as.character(pathogen)) %>%
      dplyr::select(dataset, fever:stomach_pains)
    
    dfc <- bind_rows(df3, df1, df2) |>
      mutate(dataset = factor(dataset, levels = c("4 days", "7 days", "10 days")))
  } else if (without_coinf) {
    df_cols <- c("With co-infections" = "#3474b4", "Without co-infections" = "#d31d1d") 
    
    dfc <- bind_rows(symptoms_scaled[i,] |>
                       mutate(dataset = "With co-infections",
                              pathogen = as.character(pathogen)) |>
                       dplyr::select(dataset, fever:stomach_pains),
                     symptoms_scaled_woci[i,]|> 
                       mutate(dataset = "Without co-infections", 
                              pathogen = as.character(pathogen)) |>
                       dplyr::select(dataset, fever:stomach_pains))
  }
  
  radar_plot <- ggradar(dfc,
                        group.colours = df_cols,
                        group.point.size = 0,
                        grid.min = 0, grid.mid = 0.5, grid.max = 1,
                        base.size = 0.000001,
                        grid.label.size = 3,
    ...
    theme(
      ...
      legend.text = element_text(
        size = 8, 
        colour = if (i == 2) "black" else "white"
      ),
    ...
    guides(color = guide_legend(
      override.aes = list(
        size = 1,
        color = if (i != 2) rep("white", length(df_cols)) else df_cols
      )
    ))
...
radar_plot_8 <- data.frame(
  ...
  theme(
    ...
    legend.key.size = ggplot2::unit(0.0, "cm"),
    plot.title = element_text(size = 8, face = "bold", hjust = 0.5),
    axis.text = element_text(size = 6),
    panel.grid = element_blank()) +
  guides(color = guide_legend(
...
if (!without_coinf) {
  jpeg("../../mcid3_DK/Draft/figure4_sens1.jpeg", width = 18, 
       height = 20, units = 'cm', res = 500)
  gridExtra::grid.arrange(grobs = spider, ncol = 2)
  dev.off()
} else if (without_coinf) {
  jpeg("../../mcid3_DK/Draft/figure4_sens2.jpeg", width = 18, 
       height = 20, units = 'cm', res = 500)
  gridExtra::grid.arrange(grobs = spider, ncol = 2)
  dev.off()
}
